...
Every SciForge skill accepts an optional **effort** parameter that controls how much work the system does. This affects breadth, depth, iterations, and coverage — but **never** the quality of cross-model review.

Acceptable values: `lite` | `balanced` | `max` | `beast`.
...
| Setting | Value | Why |
|---------|-------|-----|
| Reviewer reasoning effort | **maximum** | Reviewer quality is non-negotiable |
...
The reviewer's reasoning effort stays at maximum regardless of the
caller's effort level. This is the contract that protects review quality
when the user dials depth down for budget reasons.
...
Current SciForge behavior. What existing users get today. No breakage.
...
Acceptable values: `draft` | `submission`.
...
Each skill declares its effort constant in its Constants section:

> **EFFORT = `balanced`** — Work intensity. Options: `lite`, `balanced`, `max`, `beast`. Override by setting the effort level for the run.

Numeric constants in the skill are then derived from the active effort
level. Example mapping:

| Effort | MAX_PAPERS | MAX_IDEAS | MAX_ROUNDS |
|--------|-----------|-----------|------------|
| `lite` | 8 | 6 | 2 |
| `balanced` | 15 | 12 | 4 |
| `max` | 25 | 16 | 6 |
...
If the effort level is not specified for a run, default to `balanced`.

## Transparency

Every skill should print its effort configuration at the start of its
work, including the active effort level, the derived numeric knobs, and a
reminder that the reviewer reasoning effort stays at maximum regardless:

> `effort: max` — papers=25, ideas=16, rounds=6 | reviewer reasoning effort: maximum (always)

## Precedence

Knob precedence, from highest to lowest:

1. Explicit concrete knob (e.g., `review_rounds: 2`)
2. Explicit dimension override
3. Overall effort level
4. Skill default (`balanced`)

Example: `effort: beast` with `review_rounds: 3` → everything beast except review capped at 3.
...
Example: `effort: balanced` with `assurance: submission` → normal depth knobs but submission-gate audit enforcement.
