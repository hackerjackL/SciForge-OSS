...
> **Status (v2.8 — mid-term M1)**: Defines how the orchestrator dynamically adjusts **Phase 5 (method-registry) / Phase 6 (theory-derivation) / Phase 11 (unified-plotting)** intensity based on the `evidence_type` + `reasoning_paradigm` written to `refine-logs/domain-signature.json` by `/domain-learner` (Phase 1b). This replaces the v2.7 "all phases MUST at uniform strength" design with a learner-driven adaptive pipeline — the orchestrator reads the signature's `evidence_type` and `reasoning_paradigm` fields and applies the override table below before launching Phase 5/6/11.
>
