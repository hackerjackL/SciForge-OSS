...
> **Status (v2.8 — mid-term M1, v2.9 — cross-link to M3)**: Defines how the orchestrator dynamically adjusts **Phase 5 (method-registry) / Phase 6 (theory-derivation) / Phase 11 (unified-plotting)** **intensity** (REDUCED/STANDARD/INTENSIFIED/REPLACED/SKIPPED) based on the `evidence_type` + `reasoning_paradigm` written to `refine-logs/domain-signature.json` by `/domain-learner` (Phase 1b). This file adapts **intensity**; the orthogonal companion [`pipeline-adaptive-degradation.md`](pipeline-adaptive-degradation.md) (M3) adapts **mode** (MUST/CONDITIONAL/OPTIONAL/SKIP). The two files are independent — read either depending on which axis the orchestrator is configuring.
>
