...
## Figure Audit (submission gate)

When `assurance=submission`, the verifier additionally checks:

### FIGURES_VECTOR

- **Rule**: All `\includegraphics` targets in `paper/main.tex` and `paper/sections/*.tex` must reference `.pdf` or `.svg` files.
- **Check**: Parse `\includegraphics{...}` commands, verify each target extension is `.pdf` or `.svg`.
- **Verdict**: `FAIL` if any `.png`/`.jpg`/`.jpeg`/`.gif` found in main body figures.
- **Exception**: `.png` is allowed ONLY in `paper-poster` and `paper-slides` (PPTX embedding), NOT in paper body.
...
### FIGURES_DATA_CONSISTENCY

- **Rule**: Data figures must reference data sources that exist in `results/` or `data/`.
- **Check**: Parse `gen_*.py` scripts for data file paths, verify those paths exist.
- **Verdict**: `FAIL` if any referenced data file is missing and no alternative source is documented; `WARN` if data source is external/remote.
