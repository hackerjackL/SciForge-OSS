...
| Source | Best Use | Strengths | Caveats |
|--------|----------|-----------|---------|
| APS Journals | PRL/PRB/PRX/PRApplied papers, BibTeX retrieval | Official APS metadata, structured BibTeX with DOI | Requires subscription or API access |
| Optica InfoBase | Optics Express/Optica Letters/Optica papers | Official Optica Publishing Group metadata | DOI-based access |
| SPIE Digital Library | SPIE proceedings and journals | Conference proceedings in optics/photonics | Less common for journal papers |
| IOPscience | IOP journals (JPhys, Nanotechnology, etc.) | Strong physics journal coverage | Some journals behind paywall |
| arXiv physics.* | Physics preprints | Almost all physics papers appear here first | Not a substitute for published version |
| INSPIRE-HEP | High-energy physics, particle physics | Authoritative database for HEP community | Specialized to HEP only |

### Economics Sources

| Source | Best Use | Strengths | Caveats |
|--------|----------|-----------|---------|
| NBER | Working papers, BibTeX retrieval | Authoritative source for economics working papers | Working papers only, not final published versions |
| RePEc / IDEAS | Economics paper search, BibTeX retrieval | Comprehensive economics bibliographic database | Metadata quality varies |
| EconLit | Academic economics literature | Curated by AEA, high-quality structured metadata | Requires subscription (typically via institution) |
| AEA Journals | AER, QJE, JPE, AEJ family papers | Official AEA metadata with DOI | Limited to AEA journals |
| SSRN | Working papers, preprints | Broad social science coverage | Metadata quality varies widely |

**Economics decision logic**:
```text
Need to search economics papers -> RePEc / IDEAS
Already have a NBER working paper -> NBER (for working paper version) + CrossRef (for published version)
Need curated metadata -> EconLit (if available)
Need official AEA journal metadata -> AEA Journals
Need working paper / preprint -> SSRN
```
...
The protocol is invoked by `/research-lit` (mandatory filter between SEARCH
and VERIFY), and is referenced by `/idea-creator` and `/novelty-check` as a
required filter on cited papers.
...
**Caching.** Default cache scope is per-project with 30-day TTL. Cache keys
use canonical identifier priority: `arxiv:{id_without_version}` →
...
**CrossRef User-Agent.** Set a contact email environment variable to your
institutional email to reduce CrossRef rate-limit risk. A placeholder contact
is fine for low-volume use, but CrossRef prefers a real contact for higher
polite-pool throughput.
...
`/novelty-check` — must follow). The pre-search verifier is a mandatory
filter between SEARCH and full VERIFY. If the verifier is unavailable or its
invocation fails, callers must NOT silently skip filtering. Instead, emit a
degraded output tagging every candidate as `status=unverified, method=none`
with an explicit WARN, so the unverified set remains visible for downstream
audit. The degraded output preserves the same schema as the verifier's
normal output (see below).

`candidate_papers.json` schema (input):
...
`verified_papers.json` schema (output — top-level `verdict` + per-paper
`status`; the degraded fallback uses the same shape with all papers tagged
`status=unverified, method=none` and `verdict=WARN`):

```json
{
  "verdict": "PASS",
  "summary": "3 candidates, 2 verified, 1 unverified",
  "papers": [
    {"id": "p1", "status": "verified", "method": "arxiv"},
    {"id": "p2", "status": "verified", "method": "crossref"},
    {"id": "p3", "status": "unverified", "method": "none"}
  ]
}
```
...
This workflow still prioritizes compatibility with existing conference templates. It does not force a move to BibLaTeX.
