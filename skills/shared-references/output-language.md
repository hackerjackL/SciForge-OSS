...
1. Check `AGENT_DOC.md` for a `language:` field in `## Pipeline Status` — if `language: zh` or `language: cn`, output in Chinese
...
- **Machine-parsed markers** — never localize the following, regardless of language setting:
  ...
  - Research Wiki schema fields (e.g., `outcome: negative`, `outcome: positive`, `node_id:`) parsed by the wiki skill
