...
|---|---|---|---|---|---|
| `methods/METHOD_REGISTRY.md` | methods/ | `/method-registry` (canonical, economics overlay) | `/econometrics-tools`, `/leakage-audit`, `/experiment-bridge`, `/result-to-claim`, `/auto-review-loop` (econ variant), `/paper-plan`, `/paper-write`, `/invariant-check`, `/paper-claim-audit` | Sections 1-8 mandatory; Section 3 immutable post-lock (Method Selection = hash lock target) | `/invariant-check` at every phase entry |
| `methods/REGISTRY_HASH.txt` | methods/ | `/method-registry` (canonical, economics overlay) | `/result-to-claim`, `/invariant-check`, `/paper-claim-audit`, `/leakage-audit` (re-audit), replication `run_all.sh` | Single-line SHA256 hex | `/invariant-check` recomputes and compares |

| `methods/OUTCOME_CLASSIFICATION.md` | methods/ | `/method-registry` (canonical, economics overlay, Step 9) | `/result-to-claim` (Step 0 + 3.2), `/invariant-check` INV-E2, `/auto-review-loop`, `/paper-write`, `/paper-claim-audit`, replication package | Primary + Secondary outcome tables; REGISTRY_HASH pointer; locked flag | `/invariant-check` INV-E2 verifies existence and both sections non-empty |
| `methods/METHOD_BINDING.md` | methods/ | `/method-registry` (canonical, Step 8) | `/experiment-bridge` (Phase 1.7 verification), `/result-to-claim` (Step 3.2 estimator cross-check), `/paper-claim-audit` (failure mode 11: estimator_name_mismatch), `/paper-write` (Empirical Strategy section uses bound names), `/invariant-check` (econ invariant INV-E5) | Selected Methods table + Lock Status + Binding Hash; LOCKED state is immutable post-lock | `/experiment-bridge` Phase 1.7 rejects EXPERIMENT_PLAN.md on LOCKED-binding mismatch; `/invariant-check` INV-E5 verifies existence |
...
| `review-stage/REVIEW_LEDGER.json` | review-stage/ | `/auto-review-loop` (Phase E.5 per round; finalized at Termination) | `/paper-claim-audit` (cross-checks review history without parsing prose), `/paper-write` Phase 0.5 (Economics Pre-Submission Gate reads statistical_gate status), external verifier | 6-state verdict schema in `assurance-contract.md` extended with `details.rounds[]` array (per-round: score, verdict, action_items, debate_rulings, statistical_gate) | External verifier `verify_review_ledger.sh` checks existence and per-round schema; `/paper-claim-audit` treats missing ledger as `WARN: missing_review_ledger` |
| `paper/main.pdf` + `paper/main.tex` + `paper/sections/*.tex` | paper/ | `/paper-write` (compiled by `/paper-compile`) | `/auto-paper-improvement-loop`, `/proof-checker`, `/paper-claim-audit`, `/citation-audit`, `/kill-argument`, `/render-html` | LaTeX compilation = 0 warnings | `/paper-compile` is the gate |
| `paper/PAPER_CLAIM_AUDIT.{md,json}` | paper/ | `/paper-claim-audit` | `/auto-paper-improvement-loop`, `/paper-writing` Phase 6 verifier | 6-state verdict schema in `assurance-contract.md` | External verifier `verify_paper_audits.sh` |
...
| `paper/PROOF_AUDIT.{md,json}` | paper/ | `/proof-checker` | `/paper-writing` Phase 6 verifier | 6-state verdict schema in `assurance-contract.md` | External verifier `verify_paper_audits.sh` |
...
| `AGENT_DOC.md` | project root | `/economics-empirical-pipeline` Phase 0 (and equivalent Phase 0 in other pipelines) | Every downstream phase (re-reads at phase entry to confirm config intact) | Discipline / venue / methodology / gate settings table | `/invariant-check` validates discipline-specific fields |
| `replication/run_all.sh` + `VERIFICATION_REPORT.md` | replication/ | `/econometrics-tools` (replication sub-command) | AER / openICPSR submission | Bash script must exit 0; VERIFICATION_REPORT.md must confirm every figure/table reproducible | `run_all.sh` itself verifies `REGISTRY_HASH` |
...
| `data/<dataset_name>/DATA_PROVENANCE.json` (real) OR `data/simulated/DATA_PROVENANCE.json` (synthetic) | data/ | `/data-acquisition` (Phase 3 step 4 for real; Phase 4 step 3 for synthetic) | `/ouroboros-data-insight` (selects real-vs-synthetic profiling branch), `/paper-claim-audit` (failure mode 13 reads `data_type`), `/auto-review-loop` (statistical gate reads `data_type` as BLOCK override for empirical claims on synthetic), `/invariant-check` (physics overlay verifies `physical_validity` for synthetic), `/idea-creator` (overclaim_risk flag when `data_type: synthetic`) | `data_type: real\|synthetic` field is MANDATORY; real schema adds `source_url`/`license`/`acquisition_method`; synthetic schema adds `generator_skill`/`simulation_config`/`random_seed`/`validation_status`/`physical_validity`; both end with `provenance_hash: sha256:<hash>` |
| `data_acquisition_report.md` | project root | `/data-acquisition` | `/invariant-check` (INV-E4 reads DATA_SOURCE flag), `/paper-claim-audit` (failure mode 13 reads data_type) | markdown with DATA_SOURCE flag (real\|synthetic), provenance details | |
| `data_analysis/.cache/insight_cache.json` | data_analysis/.cache/ | `/ouroboros-data-insight` (Phase 7 writes; Phase 0 reads) | `/ouroboros-data-insight` (self-consumed across invocations — Phase 0 of next run checks hash match to decide skip-vs-rerun) | `schema_version` + `input_hash` (data_files/research_brief/ref_paper_summary) + `data_type` + `stopping_rule` + `data_profile_summary` (6-dim) + `artifacts` (per-file SHA256) | Phase 0 hash-mismatch auto-invalidates; read-only for all other skills |
| `data_analysis/DATA_INSIGHT_REPORT.md` | data_analysis/ | `/ouroboros-data-insight` | `/idea-creator` (MANDATORY input — hard filter on CONSTRAINED/BLOCKED axes) | ≤500 words; 6-dim profile; top 3 supportive; top 3 restrictive; info ceiling; one-sentence advisory |
| `data_analysis/final_reference_report.md` | data_analysis/ | `/ouroboros-data-insight` | advisory only; user review | Full 7-8 file structured report |
| `idea-stage/IDEA_REPORT.md` | idea-stage/ | `/idea-creator` | `/novelty-check`, `/research-review`, `/research-refine-pipeline`, `/paper-plan` | Ranked idea list + per-idea AIM Sketch (economics) or PNV Sketch (physics) |
| `idea-stage/IDEA_DAG.json` | idea-stage/ | `/idea-creator` Phase 2.5 (DAG-Based Idea Search) | `/idea-creator` (self-consumed across MCTS iterations), `/novelty-check` (reads promoted ideas), `/research-review` (reads promoted ideas) | DAG JSON v1.0 per [`idea-dag-schema.md`](idea-dag-schema.md): nodes with fidelity_scores + ucb_score + visits; edges (mutation/crossover); search_metadata |
...
| `refine-logs/FINAL_PROPOSAL.md` | refine-logs/ | `/research-refine-pipeline` | `/method-registry`, `/experiment-bridge`, `/paper-plan` | Problem Anchor frozen section (idea-confirmed state) |
| `EXPERIMENT_PLAN.md` + `EXPERIMENT_TRACKER.md` | refine-logs/ | `/experiment-plan` (canonical), `/idea-discovery` Phase 4.5, `/research-refine-pipeline` | `/experiment-bridge`, `/ablation-planner`, `/experiment-audit`, `/result-to-claim` | Plan = roadmap with claim map + blocks + run order; Tracker = state machine (Run ID/Milestone/Purpose/Status) |
| `CLAIMS_FROM_RESULTS.md` | project root | `/result-to-claim` | `/paper-plan` (starting point of claims), `/paper-claim-audit` (failure mode 10: significance_gate_violation), `/paper-write` Phase 0.5 (Economics Pre-Submission Gate), `/invariant-check` (INV-E2) | Per-claim verdict + significance gate (yes/partial/no) + REGISTRY_HASH pointer; Claims-Evidence Matrix |
| `NARRATIVE_REPORT.md` | project root | `/economics-empirical-pipeline` Phase 7.7, `/research-pipeline` Stage 4, `/physics-pipeline` Phase 7.7 | `/paper-plan`, `/paper-write`, `/paper-writing` | Structured research narrative with claims and evidence; discipline-aware (economics=AIM chain / cs-ml=SOTA narrative / physics=PNV chain) |
| `GAP_REPORT.md` | project root | `/paper-plan` | `/paper-write` (reads DATA_NEEDED markers), `/paper-writing` | List of evidence gaps blocking paper completion; one row per gap with claim_id, missing_evidence, suggested_action |
| `SIMULATION_CONFIG.md` | methods/ | `/physics-pipeline` Phase 3 | `/experiment-bridge` (Phase 1.8 reads convergence study plan) | Solver config: software+version, mesh/grid, boundary conditions, convergence criteria, pseudopotentials, system size, code availability, benchmarking |
| `PAPER_PLAN.md` | project root | `/paper-plan` | `/paper-figure`, `/paper-write`, `/paper-compile` | Section plan + figure plan + citation plan |
| `MANIFEST.md` | project root | every skill appends | pre-flight check at every skill entry | `output-manifest.md` schema |
| `EXPERIMENT_LOG.md` | project root | `/experiment-bridge` | `/result-to-claim`, `/auto-review-loop`, replication | Run-by-run append-only log |
| `figures/latex_includes.tex` | figures/ | `/paper-figure` | `/paper-write`, `/paper-compile` | Valid LaTeX `\includegraphics` commands referencing `.pdf` files | `/paper-compile` (compilation → figure reference resolution) |
| `figures/d2_output/<name>.d2` + `<name>.pdf` | figures/d2_output/ | `/d2-diagram` | `/paper-write`, `/paper-compile`, `/drawio-export` | D2 source compiles without error; PDF output is valid vector | `/paper-compile` (figure reference resolution) |
| `figures/specs/<name>.json` + `figures/<name>.svg` or `.pdf` | figures/specs/ + figures/ | `/figure-spec` | `/paper-write`, `/paper-compile` | FigureSpec JSON schema valid per `figure-spec/SKILL.md`; SVG is valid vector | `/paper-compile` (figure reference resolution) |
| `figures/<name>.drawio` | figures/ | `/drawio-export` | User (GUI editing via draw.io) | Valid .drawio XML; exported PDF is vector | Manual (human review) |
...
| Artifact | Path | Producer | Consumers | Schema enforced by | Verifier |
|---|---|---|---|---|---|---|
| `methods/METHOD_REGISTRY.md` | methods/ | `/method-registry` | `/leakage-audit`, `/result-to-claim`, `/auto-review-loop`, `/invariant-check`, `/econometrics-tools` | 8-section schema (discipline-aware: economics=AIM / cs-ml=SOTA-targeted / physics=PNV / general=Method Sketch); Section 3 hash-locked | `/invariant-check` recomputes REGISTRY_HASH and compares |
...
| `methods/METHOD_BINDING.md` | methods/ | `/method-registry` (canonical, Step 8) | `/econometrics-tools` (Phase 9 input), `/experiment-bridge` (Phase 1.7 verification), `/leakage-audit`, `/result-to-claim`, `/paper-claim-audit` (failure mode 11), `/invariant-check` (INV-E5 / INV-C1) | Selected Methods table + Lock Status + Binding Hash; LOCKED state immutable post-lock | `/experiment-bridge` rejects on LOCKED-binding mismatch; `/invariant-check` INV-E5 verifies existence |
| `methods/OUTCOME_CLASSIFICATION.md` | methods/ | `/method-registry` (canonical, Section 4 extraction) | `/result-to-claim` (Step 0 + 3.2 primary/secondary gate), `/invariant-check` INV-E2, `/auto-review-loop`, `/paper-write`, `/paper-claim-audit` | Primary + Secondary outcome tables; REGISTRY_HASH pointer; locked flag | `/invariant-check` INV-E2 verifies existence and both sections non-empty |
| `audit_report/LEAKAGE_AUDIT.md` + `.json` | audit_report/ | `/leakage-audit` (canonical) | `/result-to-claim`, `/auto-review-loop`, `/paper-write`, `/paper-claim-audit`, `/invariant-check` | JSON schema in `leakage-audit/SKILL.md` Step 6 (discipline-aware: Type I/II/III/IV detection); 6-state verdict | `/result-to-claim` rejects if `verdict==FAIL`; `/invariant-check` re-verifies verdict consistency |
...
| `methods/BENCHMARK_BINDING.md` | methods/ | `/experiment-bridge` (Phase 1.7', CS/ML only) | `/invariant-check` (INV-C1), `/result-to-claim` (Step 3.2' SOTA Gate input) | Locked benchmark protocol: dataset version + split hash + metric computation code hash + seed strategy + baseline checkpoint hash; SHA256 hash lock | `/invariant-check` INV-C1 verifies existence and hash match; drift detection on any field change |
| `methods/ESTIMATOR_VERIFICATION.md` | methods/ | `/econometrics-tools` (Phase 9, economics only) | `/leakage-audit` (Type II pitfall context: IV first-stage F < 10 → weak-IV CRITICAL), `/invariant-check` (INV-E5) | Per-estimator pass/fail table: Estimator / Impl match / SE correct / F-stat / Pre-trend / Conley SE / Romano-Wolf / Verdict | `/invariant-check` INV-E5 verifies estimator implementation matches pre-registered binding |
...
| `EXPERIMENT_AUDIT.md` + `.json` | project root | `/experiment-audit` | `/result-to-claim` (audit context), `/paper-write` (experiment section grounding), `/paper-claim-audit` (failure mode context) | Per-experiment audit checks: reproducibility, data integrity, compute environment; 6-state verdict | Advisory, never blocking |
